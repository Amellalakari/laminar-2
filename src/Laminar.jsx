...
.lam label{display:block;font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--ink2);margin-bottom:4px;font-family:'IBM Plex Mono',monospace}
...
.stream{animation:drift 2.4s linear infinite}
...
const CELLS = {
  huvec:   { n: "Endothelial — vascular (HUVEC/HAEC)", d: 15, rho: 1060, lo: 5,    hi: 20,  acute: 60, seedH: 2,  coat: "0.1% gelatin, or fibronectin 10–50 µg/mL, 1 h 37 °C" },
  lec:     { n: "Endothelial — lymphatic / microvascular", d: 14, rho: 1060, lo: 0.5, hi: 5,  acute: 20, seedH: 2,  coat: "fibronectin 20 µg/mL + collagen I" },
  caco2:   { n: "Gut epithelium (Caco-2 / organoid-derived)", d: 18, rho: 1060, lo: 0.01, hi: 0.2, acute: 2, seedH: 4, coat: "collagen I 50 µg/mL + Matrigel 1–3%" },
  alveolar:{ n: "Alveolar epithelium (A549 / primary AEC)", d: 15, rho: 1060, lo: 0.02, hi: 0.5, acute: 3, seedH: 4, coat: "collagen IV + fibronectin, or 5% Matrigel" },
  ptec:    { n: "Kidney proximal tubule (RPTEC/HK-2)", d: 16, rho: 1060, lo: 0.2, hi: 1.0, acute: 5, seedH: 4, coat: "collagen IV 30 µg/mL" },
  hepato:  { n: "Hepatocyte / HepG2", d: 20, rho: 1070, lo: 0.01, hi: 0.5, acute: 2, seedH: 4, coat: "collagen I sandwich, or Matrigel" },
  ipsccm:  { n: "Cardiomyocyte (iPSC-CM)", d: 20, rho: 1060, lo: 0.05, hi: 0.5, acute: 2, seedH: 6, coat: "Matrigel / laminin-521" },
  neuron:  { n: "Neuron / NPC", d: 12, rho: 1050, lo: 0.005, hi: 0.1, acute: 0.5, seedH: 12, coat: "PLO + laminin, overnight" },
  msc:     { n: "MSC / fibroblast", d: 18, rho: 1060, lo: 0.5, hi: 5, acute: 20, seedH: 2, coat: "none needed on TCP; fibronectin on PDMS" },
  tumor:   { n: "Tumour line (generic adherent)", d: 17, rho: 1060, lo: 0.1, hi: 2, acute: 10, seedH: 3, coat: "fibronectin or collagen I" },
  rbc:     { n: "Red blood cell / whole blood", d: 8, rho: 1100, lo: 1, hi: 40, acute: 150, seedH: 0, coat: "BSA 1% passivation" },
};
...
function compute(i) {
  ...
  const L = i.L / 1e3;                                  // m
  const b = Math.max(w, h), a = Math.min(w, h);        // b = wide, a = short
  const Qact = act === "b" ? i.Q2 : i.Q;
  const Q = (Qact * 1e-9) / 60;                         // m³/s
  const mu = i.mu / 1000;                               // Pa·s
  const A = w * h;
  const U = A > 0 ? Q / A : 0;
  const Dh = (2 * w * h) / (w + h);
  const Re = (RHO_M * U * Dh) / mu;
  const corr = 1 / (1 - 0.63 * (a / b));
  const tauPa = (6 * mu * Q) / (b * a * a) * corr;
  const tau = tauPa * PA_TO_DYN;                        // dyn/cm²
  const dPchip = (12 * mu * L * Q) / (b * Math.pow(a, 3)) * corr;
  ...
  const floorCm2 = b * L * 1e4;
  const seedN = i.seedDens * floorCm2;                  // cells needed
  const seedConc = Vch > 0 ? seedN / (Vch * 1e3) : 0;   // cells per mL

  const dPcap = 2 * GAMMA * (1 / w + 1 / h);            // Pa, bubble pinning
...
const DEFAULTS = {
  ...
  coating: "fibronectin", coatDelay: 1, attachH: 2, seedDens: 100000,
...
${contextBlock()}`,
      ...
      if (!r || !r.ok) r = await post("https://api.anthropic.com/v1/messages");
      ...
      setMsgs([...next, { role: "assistant", text: "Could not reach the model. Check the connection and retry." }]);
  ...
  const sx = drawW / (i.w || 1);            // px per µm horizontally
  const rawH = i.h * sx;
  const exagg = rawH < 46 ? 46 / rawH : 1;
  const chH = rawH * exagg;
  const VH = chH + 76;
  const cellR = (m.dCell * 1e6 * sx * Math.sqrt(exagg)) / 2;
  const profile = [];
  for (let k = 0; k <= 20; k++) {
    const z = k / 20;
    const u = 6 * z * (1 - z);
    profile.push([PAD + 8 + u * (drawW * 0.34), 40 + z * chH]);
  }

  const lvlColor = (l) => (l === L_LIKELY ? "var(--alert)" : l === L_MAYBE ? "var(--warn)" : "var(--ink2)");

  const Field = ({ label, children }) => (<div style={{ marginBottom: 10 }}><label>{label}</label>{children}</div>);
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto", padding: "22px 16px 60px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(260px, 320px) 1fr", gap: 18, alignItems: "start" }} className="grid-wrap">
          ...
          <aside className="card" style={{ padding: 16 }}>
            ...
            <Field label={i.format === "membrane" ? "Apical channel — width × height µm" : "Width µm"}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <input type="number" value={i.w} onChange={set("w")} />
                <input type="number" value={i.h} onChange={set("h")} />
              </div>
            </Field>
            {i.format === "membrane" && (
              <>
                <Field label="Basal channel — width × height µm">
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <input type="number" value={i.w2} onChange={set("w2")} />
                    <input type="number" value={i.h2} onChange={set("h2")} />
                  </div>
                </Field>
                <Field label="Basal flow µL/min"><input type="number" step="0.1" value={i.Q2} onChange={set("Q2")} /></Field>
                ...
                <Field label="Membrane">
                  <select value={i.memMat} onChange={set("memMat")}>
                    {Object.entries(MEMBRANES).map(([k, v]) => <option key={k} value={k}>{v.n}</option>)}
                  </select>
                </Field>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                  <Field label="Pore µm"><input type="number" step="0.1" value={i.poreD} onChange={set("poreD")} /></Field>
                  <Field label="Open %"><input type="number" step="1" value={i.poreEps} onChange={set("poreEps")} /></Field>
                  <Field label="Thick µm"><input type="number" value={i.memT} onChange={set("memT")} /></Field>
                </div>
                <Field label="Outlet height offset mm (apical − basal)">
                  <input type="number" step="1" value={i.outletOffsetMM} onChange={set("outletOffsetMM")} />
                </Field>
              </>
            )}
            <Field label="Length mm"><input type="number" value={i.L} onChange={set("L")} /></Field>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label={i.format === "membrane" ? "Apical Q µL/min" : "Q µL/min"}><input type="number" step="0.1" value={i.Q} onChange={set("Q")} /></Field>
              <Field label="Viscosity mPa·s"><input type="number" step="0.01" value={i.mu} onChange={set("mu")} /></Field>
            </div>
            <Field label="Drive">
            ...
            {(i.drive === "gravity" || i.drive === "rocker") && (
              <Field label="Head height mm"><input type="number" value={i.headMM} onChange={set("headMM")} /></Field>
            )}
            {i.drive.startsWith("syringe") && (
              <Field label="Syringe mL"><input type="number" value={i.syringeML} onChange={set("syringeML")} /></Field>
            )}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Tubing ID mm"><input type="number" step="0.05" value={i.tubID} onChange={set("tubID")} /></Field>
              <Field label="Tubing cm"><input type="number" value={i.tubL} onChange={set("tubL")} /></Field>
            </div>
            <label style={{ display: "flex", gap: 8, alignItems: "center", textTransform: "none", letterSpacing: 0, fontSize: 12 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Cell dia µm"><input type="number" value={i.cellD} onChange={set("cellD")} /></Field>
              <Field label="Seed cells/cm²"><input type="number" step="10000" value={i.seedDens} onChange={set("seedDens")} /></Field>
            </div>
            <Field label="Coating">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <Field label="Coat delay h"><input type="number" step="0.5" value={i.coatDelay} onChange={set("coatDelay")} /></Field>
              <Field label="Static attach h"><input type="number" step="0.5" value={i.attachH} onChange={set("attachH")} /></Field>
            </div>
          </aside>
          ...
          <main style={{ display: "grid", gap: 18 }}>
            ...
            {/* signature: true-scale cross-section */}
            <section className="card" style={{ padding: "14px 16px 6px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", flexWrap: "wrap", gap: 8 }}>
                <div className="eyebrow">Cross-section, true horizontal scale{exagg > 1 ? ` · height exaggerated ×${sig(exagg, 2)}` : ""}</div>
                <div className="mono" style={{ fontSize: 11, color: "var(--ink2)" }}>{i.w} × {i.h} µm · {sig(m.ar, 2)}:1</div>
              </div>
              <svg viewBox={`0 0 ${VW} ${VH}`} style={{ width: "100%", height: "auto", display: "block", marginTop: 6 }} role="img"
                aria-label={`Channel cross section ${i.w} by ${i.h} micrometres with parabolic velocity profile`}>
                <defs>
                  <linearGradient id="vel" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="var(--flow)" stopOpacity="0.05" />
                    <stop offset="50%" stopColor="var(--flow)" stopOpacity="0.28" />
                    <stop offset="100%" stopColor="var(--flow)" stopOpacity="0.05" />
                  </linearGradient>
                </defs>
                {/* bulk */}
                <rect x={PAD - 22} y={18} width={drawW + 44} height={chH + 44} fill="#DFE3D9" stroke="var(--line)" />
                {/* channel */}
                <rect x={PAD} y={40} width={drawW} height={chH} fill="url(#vel)" stroke="var(--flow)" strokeWidth="1" />
                {/* streamlines */}
                {[0.2, 0.35, 0.5, 0.65, 0.8].map((z, k) => (
                  <line key={k} className="stream" x1={PAD + 4} x2={PAD + drawW - 4} y1={40 + z * chH} y2={40 + z * chH}
                    stroke="var(--flow)" strokeWidth={1.4 * (1 - Math.abs(z - 0.5) * 1.4)} strokeOpacity="0.5"
                    strokeDasharray="10 30" />
                ))}
                {/* velocity profile */}
                <polyline points={profile.map((p) => p.join(",")).join(" ")} fill="none" stroke="var(--dye)" strokeWidth="1.8" />
                <line x1={PAD + 8} y1={40} x2={PAD + 8} y2={40 + chH} stroke="var(--dye)" strokeWidth="0.8" strokeDasharray="2 3" />
                {/* cells on floor */}
                {Array.from({ length: Math.min(26, Math.max(4, Math.floor(drawW / (cellR * 2.6 || 20)))) }).map((_, k, arr) => (
                  <circle key={k} cx={PAD + 14 + k * ((drawW - 28) / arr.length)} cy={40 + chH - Math.max(cellR, 1.2)}
                    r={Math.max(cellR, 1.2)} fill="var(--dye)" fillOpacity="0.55" stroke="var(--dye)" strokeWidth="0.6" />
                ))}
                {/* dims */}
                <text x={PAD} y={VH - 8} className="mono" fontSize="10" fill="var(--ink2)">◄ {i.w} µm ►</text>
                <text x={VW - PAD} y={VH - 8} textAnchor="end" className="mono" fontSize="10" fill="var(--dye)">
                  cell ⌀{sig(m.dCell * 1e6)} µm sits in {sig((m.uCellTop / m.U) * 100, 2)}% of mean velocity
                </text>
              </svg>
            </section>
            ...
            <section className="card">
              <div style={{ display: "flex", borderBottom: "1px solid var(--line)" }}>
                {[["diagnose", "Diagnose"], ["chat", "Brainstorm"], ["log", `Case log${cases.length ? ` · ${cases.length}` : ""}`]].map(([k, l]) => (
            ...
            <p className="mono" style={{ fontSize: 10.5, color: "var(--ink2)", lineHeight: 1.7, margin: 0 }}>
              Shear uses τ = 6µQ/wh² with the (1 − 0.63 h/w) rectangular-duct correction; valid for fully developed laminar flow in a straight duct.
